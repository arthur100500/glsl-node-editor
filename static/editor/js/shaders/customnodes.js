// groupsNnodes = [[grname1, [code1, add1], [code2, add2], ..], [grname2, [code4, add4], [code5, add5], ..], [..], [..], ..];
function setNodesToContextMenu(groupsNnodes) {
  contextMenuNodes.innerHTML = '';
  let innerInnerDiv;
  const appendNodeDiv = document.createElement('div');
  document.querySelector('#new-node-btn').onmousedown = function () {
    if (appendNodeDiv.parentNode !== null) { contextMenuNodes.removeChild(appendNodeDiv); } else { contextMenuNodes.appendChild(appendNodeDiv); }
  };
  ...
  for (let i = 0; i < groupsNnodes.length; i++) {
    ...
    for (let j = 1; j < groupsNnodes[i].length; j++) {
      ...
      innerInnerDiv.onmousedown = function () {
        allNodes.push(nodeFromFunction(
          groupsNnodes[i][j][0],
        ...
        allNodes[allNodes.length - 1].positionX = (mouseX - getMatrix()[4] + getScrollLeft() - xOff) / getMatrix()[0];
        allNodes[allNodes.length - 1].positionY = (mouseY - getMatrix()[5] + getScrollTop() - yOff) / getMatrix()[3];
        allNodes[allNodes.length - 1].draw(mainEditorDiv);
      };
