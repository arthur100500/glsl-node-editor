...
class RectBackground {
  ...
  getHtml() {
    const svgB = `<svg width="${this.baseNode.width}" height="${this.baseNode.height}" xmlns="http://www.w3.org/2000/svg">`;
    const svgE = '</svg>';
...
class HeaderRectBackground {
  ...
  getHtml() {
    const svgB = `<svg width="${this.baseNode.width}" height="${25}" xmlns="http://www.w3.org/2000/svg">`;
    const svgE = '</svg>';
...
class Node {
  ...
  recountGraphicParams() {
    if (this.output.type !== 'void') { this.height = 70 + (this.inputs.length + this.parameters.length) * 30; } else { this.height = 20 + (this.inputs.length + this.parameters.length) * 30; }
  }
  ...
  draw(target) {
    ...
    // Close button
    const nodeRemoveBtn = document.createElement('span');
    ...
    if (this.output.type !== 'void') {
      ...
      if (this.functional) { setAbleToConnectFrom(outputDiv, this.output); }
    }
    ...
    for (let i = 0; i < this.inputs.length; i++) {
      ...
      if (this.functional) { setAbleToConnectTo(inputDiv, this.inputs[i]); }
    }
    target.appendChild(this.nodeContainer);

    if (this.functional) { dragElement(this.nodeContainer); }
  }
  ...
  toJson() {
    const nodeJson = JSON.stringify(this, ['name', 'code', 'id', 'additionElements', 'parameters', 'positionX', 'positionY', 'width', 'height']);
    const inputsJson = [];
    const paramsJson = [];
    const outputJson = this.output.toJson();
    for (let i = 0; i < this.inputs.length; i++) { inputsJson.push(this.inputs[i].toJson()); }
    for (let i = 0; i < this.parameters.length; i++) { paramsJson.push(this.parameters[i].toJson()); }
...
class NodeInput {
  ...
  toJson() {
    ...
    const inputJson = JSON.stringify(this, ['type', 'name', 'id']);
    const connectedSourceJson = this.connectedSource !== null ? this.connectedSource.fullName() : 'null';
  ...
  recalculateVisualConnections() {
    ...
    const bRect = this.connectedSource.baseNode.nodeContainer.style;
    const selfBRect = this.connectedSource.divRepr.style;
    const x0 = Number(bRect.left.replace('px', '')) + Number(selfBRect.left.replace('px', ''));
    const y0 = Number(bRect.top.replace('px', '')) + Number(selfBRect.top.replace('px', ''));
    const nodeBRect = this.baseNode.nodeContainer.style;
    const sourceBRect = this.divRepr.style;
    const x1 = Number(nodeBRect.left.replace('px', '')) + Number(sourceBRect.left.replace('px', ''));
    const y1 = Number(nodeBRect.top.replace('px', '')) + Number(sourceBRect.top.replace('px', ''));
  ...
  connectSource(source) {
    ...
    if (this.connectedSource !== null) { mainEditorDiv.removeChild(this.connectedSourceVisualConnection.genElement()); }

    const bRect = source.divRepr.getBoundingClientRect();
    const selfBRect = this.divRepr.getBoundingClientRect();
  ...
  getHtml() {
    ...
    // Name
    const name = `<span style="position:absolute; left:20px; top:0px;font-size: 14pt;">${this.name}(${this.type})` + '</span>';
...
class NodeOutput {
  ...
  getHtml() {
    ...
    // Name
    const name = `<span style="position:absolute; right:20px; top:0px;font-size: 14pt;">${this.name}(${this.type})` + '</span>';
