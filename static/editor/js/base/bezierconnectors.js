...
class BezierConnector {
  ...
  genBesierCode() {
    const nx0 = this.x0 - Math.min(this.x0, this.x1);
    const nx1 = this.x1 - Math.min(this.x0, this.x1);
    const ny0 = this.y0 - Math.min(this.y0, this.y1);
    const ny1 = this.y1 - Math.min(this.y0, this.y1);

    // InnerHTML is code for besier
    const svgB = `<svg width="${Math.abs(nx0 - nx1) + 2 * strokeWidth}" height="${Math.abs(ny0 - ny1) + 2 * strokeWidth}" xmlns="http://www.w3.org/2000/svg">`;
    const svgE = '</svg>';

    // For smooth transitions
    const lc = Math.abs(this.x0 - this.x1) / 2;
