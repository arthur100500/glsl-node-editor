...
function displayErrors(code) {
  const full = `precision mediump float;${code}void main(){gl_FragColor = vec4(1.0);}`;
  const compRes = createShader(gl, full, gl.FRAGMENT_SHADER);
  const isCompiled = compRes === 'success';
  if (isCompiled) {
  ...
  const errors = compRes.replaceAll('ERROR: 0:', '').trim().split('\n');
  const annotations = [];
  console.log(errors);
